 ...
 *
 * Implements:
 *   - linked-list tracking of registered container PIDs
 *   - mutex-protected list access (safe for sleepable ioctl context)
 *   - periodic RSS checks via kernel timer
 *   - soft-limit warning (once per container)
 *   - hard-limit enforcement via SIGKILL
...
/* --- Global list and mutex (see design rationale in README) --- */
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING
                   "[container_monitor] Invalid: soft > hard for container=%s\n",
                   req.container_id);
            return -EINVAL;
        }
...
static void __exit monitor_exit(void)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        printk(KERN_INFO
               "[container_monitor] Freeing on unload: container=%s pid=%d\n",
               entry->container_id, entry->pid);
        list_del(&entry->list);
        kfree(entry);
    }
